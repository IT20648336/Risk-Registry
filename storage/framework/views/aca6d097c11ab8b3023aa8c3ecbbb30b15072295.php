...
$(document).ready(function(){
 ...
 function fetch_data()
 {
  $.ajax({
   ...
   success:function(data)
   {
    var html = '';
    html += '<tr>';
    html += '<td contenteditable id="Risk_Activity"></td>';
    html += '<td contenteditable id="last_name"></td>';
    html += '<td><button type="button" class="btn btn-success btn-xs" id="add">Add</button></td></tr>';
    for(var count=0; count < data.length; count++)
    {
     html +='<tr>';
     html +='<td contenteditable class="column_name" data-column_name="Risk_Activity" data-id="'+data[count].id+'">'+data[count].Risk_Activity+'</td>';
 ...
 function(){
  var Risk_Activity = $('#Risk_Activity').text();
  var last_name = $('#last_name').text();
  if(Risk_Activity != '' && last_name != '')
  {
   $.ajax({
    url:"<?php echo e(route('livetable.add_data')); ?>",
    method:"POST",
    data:{Risk_Activity:Risk_Activity, last_name:last_name, _token:_token},
    success:function(data)
    {
     $('#message').html(data);
     fetch_data();
...
</script><?php /**PATH /data/RiskRegistry/resources/views/RiskCreation/edit.blade.php ENDPATH**/ ?>