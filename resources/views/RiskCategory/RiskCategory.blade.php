    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ route('risk-category.store') }}">
                    @csrf
...
<div class="modal fade" id="EditUser" tabindex="-1" role="dialog" aria-labelledby="EditUserLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
               <form method="post" action="{{ route('risk-category.update') }}" id="editForm">
...
@endphp
    ...
    <tr>
       ...
       <td>
       <a href="#pageSubmenu1" data-toggle="collapse" aria-expanded="false" class="dropdown-toggle">{{ $data->Category }}</a>
        <ul class="collapse list-unstyled" id="pageSubmenu1">
          <li>
             <li class="subcategory-dropdown" data-category="{{ $data->Category }} " >
          </li>
        ...
        <td>
             <button class="button" style="height: 20px; margin-top: 1px;" type="button" onclick="openEditModal('{{ $data->Id }}', '{{ $data->Category }}')">EDIT</button>
            
        </td>
        <td>
        <label class="switch">
    @if($data->Status  == 'Active')
    <input type="checkbox"  checked onchange="changeRiskStatus('changeRiskStatus?Id={{$data->Id}}&NextStatus=Inactive'); return false;" >
    @elseif($data->Status  == 'Inactive')
    <input type="checkbox"  onchange="changeRiskStatus('changeRiskStatus?Id={{$data->Id}}&NextStatus=Active'); return false;" >
    @endif       
...
<script>
    ...
    function populateSubcategoryDropdowns() {
        ...
        Array.from(dropdowns).forEach(function(dropdown) {
            ...
            fetch('{{ route('risk-category.subcategories') }}?category=' + category)
